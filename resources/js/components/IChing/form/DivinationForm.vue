...
const emit = defineEmits<{
  // success: [coinResults: number[], question: string];
  update: [coinResults: number[], lineCount: number]; // Для обновления поля hexagramCoinResults
}>();
...
const castQuick = async () => {
  ...
  emit('update', hexagramCoinResults.value, lineCount.value);
};
...
const saveDivination = () => {
  ...
  form.post(props.submitUrl, {
    preserveScroll: true,
    // onSuccess: (page) => {
    //   emit('success', form.coin_results, form.question);
    // },
    onError: (errors) => {
        ...
        >
          ...
          <div class="space-y-2 text-center">
            ...
            <p class="text-slate-500">
              {{
                __('Would you like to record this divination in your history?')
              }}
            </p>
            <p
              v-if="isGuest"
            ...
            >
              {{ __('As a guest, you can save only one divination.') }}
            </p>
          </div>
        ...
        >
          <div v-if="mode === 'quick'" class="space-y-6 text-center">
            <div v-if="isCasting" class="space-y-4">
              <Loader2 class="mx-auto size-12 animate-spin text-amber-500" />
              <p class="animate-pulse font-serif text-xl text-amber-600 italic">
          ...
          <div v-else class="w-full max-w-md text-center">
            <div class="flex flex-col items-center">
              ...
              >
                <Coin
                  :is-spinning="castingPhase === CastingPhase.CastingSingleCoin"
              ...
              >
                <Coin
                  ...
                  :key="idx"
                  :is-spinning="castingPhase === CastingPhase.CastingThreeCoins"
                  :result="res"
                />
              </div>
